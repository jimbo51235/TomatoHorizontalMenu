...
public class TomatoHorizontalBaseController: UIViewController {
	var autoScroll = true
	...
	func makeHorizontalView(name: String, index: Int, prevWidth: CGFloat) -> UIView {
		let textColor = (index == viewIndex) ? menuHighlightColor : labelTextColor
		...
		/* label */
		let labelTopPosition = ((menuHeight - menuHighlightHeight) - labelHeight) / 2.0
	...
	@objc public func switchViewController(_ sender: UITapGestureRecognizer) {
		/*
